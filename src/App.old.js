...
const INIT_FOLIOS = [{ name: "Google", symbol: "GOOGL", currency: "USD", orders: [{ id: 1, quantity: 10, buyDate: "2019-01-01", buyPrice: 1000, buyXR: 1 }] }];
...
export default function App() {
	...
	React.useEffect(() => {
		...
		// portfolios, 
		baseCurrency, currencies, avKey, quotes, xrs]);
	...
	const handleStockEdit = (id) => {
		// let stock;
		// for (const portfolio of portfolios) {
		// 	stock = portfolio.find(stock => stock.id === id);
	...
	const handleStockSave = (stock) => {
		// const newPortfolios = portfolios.slice();
		// if (stock.id === -1) {
		// 	let maxId = Number.MIN_SAFE_INTEGER;
		// 	if (stock.portId === -1) { stock.portId = 0 }
		// 	for (const portfolio of newPortfolios) {
		// 		for (const oldStock of portfolio) {
		// 			if (oldStock.id >= maxId) { maxId = oldStock.id + 1 }
		// 		}
		// 	}
		// 	stock.id = maxId;
		// 	newPortfolios[stock.portId].push(stock);
		// } else {
		// 	for (const portfolio of newPortfolios) {
		...
		// 				replaceIndex = index;
		// 				//console.log("replace", replaceIndex, stock)
		// 				break;
		// 			}
		// 		}
		// 		if (replaceIndex !== null) {
		// 			portfolio[replaceIndex] = stock;
		// 			break;
		// 		}
		// 	}
		// }
	...
	for (const [index, folio] of folios.entries()) {
		folioArray.push(
			<Folio key={index} index={index} folio={folio}
